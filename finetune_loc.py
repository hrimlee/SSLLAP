...
..\\Results\\finetune_attnaug_v4\\ [데이터 이름]\\saved_models \\ FE : 모델 저장 경로 
..\\Results\\ finetune_attnaug_v4\\ [데이터 이름] \\ result_tex \\ : 결과 파일 저장 경로 
..\\Results\\ finetune_attnaug_v4\\ [데이터 이름] \\ result_txt \\ : 결과 파일 저장 경로 
...
class Final_predict(object):
    def __init__(self) :
        ...
        ## dir 
        self.root_dir = '..\\data\\'
        self.eeg_path = 'sleep_edfx\\EEG_test\\'  # options  [ISRUC\\EEG, HMC, sleep_edfx\\EEG_test]
        self.eog_path = 'sleep_edfx\\EOG_test\\'  # options  [ISRUC\\EOG, HMC, sleep_edfx\\EOG_test\\] 
        
        self.n_epochs = 5 # isruc 5
        self.batch_size = 64 # isruc 64
        self.kfold = False
        self.weight_name = "sleep_edfx"
        self.label_ratio = 0.2
        self.is_save = False
        self.random_seed = False
        self.info  = "simsiam_contrastive_v4: simsiam_contrastive_v2에서 attention 모델 추가 + D랑 Q구조 변경"
        self.data_name = self.eeg_path.split('\\')[0]
        self.f_name = str(inspect.getfile(inspect.currentframe())[:-3])

        if self.data_name == 'sleep_edfx': 
            self.PATH1 = glob.glob(os.path.join('.\\FE_v3\\',"netFE100_seg10.pth" )) # //60:61  80: 54.3 , 100: 55.9
            self.PATH2 = glob.glob(os.path.join('.\\Q_v3\\',"netQ100_seg10.pth" ))  # // 30epoch 80: 65
        
        elif self.kfold: # k fold weight load해서 finetune 
            self.PATH1 = glob.glob(os.path.join(f'..\\SSLAPP_results\\finetune_simsiam\\{self.weight_name}\\saved_models\\netFE\\',"netFE5*.pth" )) 
            self.PATH2 = glob.glob(os.path.join(f'..\\SSLAPP_results\\finetune_simsiam\\{self.weight_name}\\saved_models\\netQ\\',"netQ5*.pth" ))
        
        else: # 2:8 weight load해서 finetune 
            self.PATH1 = glob.glob(os.path.join(f'..\\SSLAPP_results\\{self.f_name}\\{self.weight_name}\\saved_models\\netFE\\',"netFE10_115_b64_28.pth" )) 
            self.PATH2 = glob.glob(os.path.join(f'..\\SSLAPP_results\\{self.f_name}\\{self.weight_name}\\saved_models\\netQ\\',"netQ10_115_b64_28.pth" ))
        ...
        # dir for save model 
        self.data_name = self.eeg_path.split('\\')[0] # sleep_edfx 
        
        if self.random_seed:
            self.save_dir = os.path.join("..\\Results\\", self.f_name+"_random\\",self.data_name,str(self.label_ratio)+"\\") 
        else:
        ...
        ## dataloader  
        eeg_dir = os.path.join(self.root_dir, self.eeg_path)
        
        self.data_loader = rawdata_loader.dataloader(self.root_dir, self.eeg_path, self.eog_path, batch_size = self.batch_size, normalization = True)
        #self.data_loader = rawdata_loader.dataloader(self.root_dir, self.eeg_path, self.eog_path, batch_size = self.batch_size, standardization = True)
        torch.manual_seed(0)
        # train, test 나누는 부분 ratio로 설정
        ...
        for param in self.Q_model.parameters():
            ...
            self.model_name = 'Q'+self.PATH1.split('\\')[2][8:-4]
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            for iter, (x_, y_) in enumerate(self.train_loader):
    ...
    def test(self):
        with torch.no_grad():
            ...
            for _, (x_, label) in enumerate(self.test_loader):
                x_ = x_.type(torch.FloatTensor) # type casting
            ...
            # save as txt 
            # dic로 저장하지 않고, txt에서 보기 좋게 저장하기 위해 한번더 classification report 뽑음
